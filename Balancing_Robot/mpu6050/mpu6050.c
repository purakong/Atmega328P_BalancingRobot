...
double DEGREE_PER_SEC  = 65535.0/2000.0;
...
void mpu6050_read_accel_ALL(accelVal * data){
	...
	data->accel_x = (tmp[0]<<8)|(tmp[1]);
	mpu6050_read_accel_Y(tmp);
	data->accel_y = (tmp[0]<<8)|(tmp[1]);
	mpu6050_read_accel_Z(tmp);
	data->accel_z = (tmp[0]<<8)|(tmp[1]);
}
...
void mpu6050_read_accelZ(accelVal * data){
